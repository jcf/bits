# Vendored from nix2container/default.nix with a single fix: the
# `sed -i $out` in fetchgitpatch's postFetch now writes through $TMPDIR
# and moves the result over $out. The original creates its temp file
# next to $out (inside /nix/store), which the strict Darwin sandbox
# blocks with EPERM.
#
# Upstream tracked at code.invetica.team/mirrors/nix2container. When
# bumping the mirror, diff this file against upstream's default.nix and
# carry any non-trivial changes across.
#
# The fileset filtering used by upstream for nix2container-bin's src has
# been dropped — flake inputs are already pinned revisions, so filtering
# doesn't reduce rebuilds, and `lib.fileset.gitTracked` doesn't work on
# fetchTree snapshots (no .git).
{
  pkgs ? import <nixpkgs> {},
  src,
}: let
    ...
    preBuild = let
      ...
      fetchgitpatch = args:
        pkgs.fetchpatch2 (args
          // {
            # Write through $TMPDIR rather than `sed -i`, which would create
            # its temp file next to $out (inside /nix/store) and fail under
            # the strict Darwin sandbox.
            postFetch =
              (args.postFetch or "")
              + ''
                sed \
                  ...
                  $out > $TMPDIR/normalized
                mv $TMPDIR/normalized $out
              '';
          });
      patch = fetchgitpatch {
  ...
  in
    checked {inherit copyToRoot contents;} (out // {inherit nestedLayers;});
  ...
  buildImage = {
    ...
    in
      makeNixDatabase closureGraphForAllLayers;
    ...
    imageTag = let
      hash = l.head (l.splitString "-" (baseNameOf image.outPath));
    in
      l.defaultTo hash tag;
  ...
  in
    checked {inherit copyToRoot contents;} out;
