...
**** Public/Demo Endpoints (Intentionally Unprotected)
- ~/api/handles/:handle~ (GET) - Check subdomain availability (allows users to explore before signup)
...
- [X] Identify all endpoints that require verification
- [X] Replace ~auth: AuthSession~ with ~verified: Verified~ on protected endpoints
- [X] Keep ~auth: AuthSession~ on auth/verification endpoints
- [X] Add integration test for unverified user accessing protected endpoint
- [X] Add integration test for verified user accessing protected endpoint
- [X] Document which endpoints require verification and why
- [X] Client-side handles 403 EmailNotVerified by redirecting to ~/verify-email~
