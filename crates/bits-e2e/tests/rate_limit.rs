...
async fn ip_based_login_rate_limit_blocks_excessive_attempts() {
    ...
    assert_eq!(status, 429, "IP-based rate limit should return 429 status");
}
...
async fn ip_based_registration_rate_limit_allows_more_attempts() {
    ...
    for i in 0..20 {
        ...
        let result = client.join_result(&email, "password").await;
        assert!(
            result.is_ok(),
    ...
    let result = client.join_result("user21@example.com", "password").await;
    assert!(
        result.is_err(),
...
async fn email_based_registration_rate_limit_prevents_spam() {
    ...
    let result = client.join_result(email, "password2").await;
    assert!(
        result.is_err(),
    ...
    let result = client.join_result(email, "password3").await;
    assert!(
        result.is_err(),
